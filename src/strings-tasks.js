...
function getStringLength(value) {
  ...
  return isValueString ? value.length : 0;
}
...
function repeatString(str, times) {
  const isStrLength = str.length > 0;
  const isPositiveTimesCount = times > 0;
  return isStrLength && isPositiveTimesCount ? str.repeat(times) : '';
}
...
function removeFirstOccurrences(str, value) {
  ...
  if (searchIndex !== -1) {
    return `${str.slice(0, searchIndex)}${str.slice(
...
function sumOfCodes(str) {
  const isStringParam = typeof str === 'string';
  return isStringParam
    ? str.split('').reduce((acc, letter) => acc + letter.charCodeAt(0), 0)
    : 0;
}
...
function isPalindrome(str) {
  const baseItem = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const reversedItem = baseItem.split('').reverse().join('');
  return baseItem === reversedItem;
}
